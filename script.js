...
function applyRangeSettings() {
  applyUiSetting("--app-width", `${appWidthInput.value}px`);
  applyUiSetting("--app-padding", `${appPaddingInput.value}px`);
  applyUiSetting("--panel-gap", `${panelGapInput.value}px`);
  applyUiSetting("--app-radius", `${appRadiusInput.value}px`);
  applyUiSetting("--control-radius", `${buttonRadiusInput.value}px`);
  applyUiSetting("--key-height", keyHeightInput.value);
  applyUiSetting("--display-height", `${displayHeightInput.value}px`);
  applyUiSetting("--display-font-size", displayTextInput.value);
  applyUiSetting("--key-font-size", keyTextInput.value);
  applyUiSetting("--button-gap", `${buttonGapInput.value}px`);
  applyUiSetting("--border-width", `${borderWidthInput.value}px`);
}

function resetUi() {
  Object.entries(uiDefaults).forEach(([name, value]) => {
    document.body.style.setProperty(name, value);
  });
  uiColorInputs.forEach((input) => {
    input.value = uiDefaults[input.dataset.uiColor];
  });
  appWidthInput.value = "1120";
  appPaddingInput.value = "22";
  panelGapInput.value = "18";
  appRadiusInput.value = "24";
  buttonRadiusInput.value = "16";
  keyHeightInput.value = "72";
  displayHeightInput.value = "296";
  displayTextInput.value = "76";
  keyTextInput.value = "20";
  buttonGapInput.value = "10";
  borderWidthInput.value = "1";
  fontSelect.value = uiDefaults["--font-family"];
}
...
fontSelect.addEventListener("change", () => {
  applyUiSetting("--font-family", fontSelect.value);
});

uiColorInputs.forEach((input) => {
  input.addEventListener("input", () => {
    applyUiSetting(input.dataset.uiColor, input.value);
  });
});
