...
async function updateConversion(source) {
  ...
  const converted = await convertUnit(value, type, sourceUnit, targetUnit);
  if (requestId !== conversionRequestId) return;
  targetInput.value = toInputNumber(converted);
  saveAppState();
}
...
function bindConverterInput(input, source) {
  input.addEventListener("beforeinput", (event) => {
    ...
    const nextValue = getInputValueAfterEdit(input, event.data || "");
  ...
  input.addEventListener("input", () => {
    if (!isValidConverterInput(input.value)) input.value = sanitizeConverterInput(input.value);
    updateConversion(source);
  });
}
...
function multifactorial(value, step) {
  if (!Number.isInteger(value) || value < 0 || value > 300) {
    throw new Error("Multifactorial requires an integer from 0 to 300");
  }
...
function updateToggleUi() {
  ...
  setToggle("wordCornerToggle", "wordCornerToggleText", "Appear in Corner", settings.wordFormCorner);
  setToggle("pedmasToggle", "pedmasToggleText", "PEMDAS", settings.pemdas);
  setToggle("autoMemoryToggle", "autoMemoryToggleText", "Auto Memory", settings.autoMemory);
  setToggle(
