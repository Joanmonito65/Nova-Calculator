...
function applyFactorial() {
  if (state.expression === "0") return;
  if (settings.iteratedFactorials) {
    setExpression(`${state.expression}!`);
    return;
  }
  wrapExpression("fact");
...
function toFractionString(value, improper) {
  const sign = value < 0 ? "-" : "";
  const fraction = approximateFraction(Math.abs(value), 100000);
...
function approximateFraction(value, maxDenominator) {
  ...
  for (let index = 0; index < 24; index += 1) {
    ...
    const remainder = b - a;
    if (Math.abs(value - h1 / k1) < 1e-12 || remainder === 0) break;
    b = 1 / remainder;
  }
...
[appRadiusInput, buttonRadiusInput, keyHeightInput, displayTextInput, keyTextInput].forEach((input) => {
